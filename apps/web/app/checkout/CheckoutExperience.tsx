...
const testCoupons: Record<string, { label: string; discount: (subtotal: number) => number }> = {
  YAFA20: { label: "20% off your ritual", discount: (subtotal) => Math.round(subtotal * 0.2) },
  NATURE15: { label: "15% off your ritual", discount: (subtotal) => Math.round(subtotal * 0.15) },
  FLAT500: { label: "₹500 off your ritual", discount: (subtotal) => Math.min(subtotal, 500) },
  WELCOME10: { label: "10% off your ritual", discount: (subtotal) => Math.round(subtotal * 0.1) },
};

function calculateTotals(subtotal: number, shippingMethod: ShippingMethod, discount: number) {
  const discountedSubtotal = Math.max(0, subtotal - discount);
  const shipping = shippingMethod === "express" ? 299 : (discountedSubtotal >= FREE_SHIPPING_THRESHOLD ? 0 : 199);
  return { shipping, total: discountedSubtotal + shipping };
}
...
export default function CheckoutExperience() {
  ...
  const [pinkPouch, setPinkPouch] = useState(false);
  ...
  function applyDiscount() {
    ...
    if (testCoupons[code]) { setDiscountCode(code); setDiscountMessage(`${code} applied — ${testCoupons[code].label}.`); }
    else { setDiscountCode(""); setDiscountMessage(code ? "That code is not available." : "Enter a code to apply it."); }
  }
  ...
  async function startPayment(event: FormEvent<HTMLFormElement>) {
    ...
    setState("processing");
    try {
      const orderResponse = await fetch("/api/payments/razorpay/order", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ shippingMethod, discountCode, customer: form }) });
      const order = await orderResponse.json() as { error?: string; orderId?: string; amount?: number; currency?: string; keyId?: string };
      if (!orderResponse.ok || !order.orderId || !order.amount || !order.keyId) throw new Error(order.error || "We could not prepare your payment. Please try again.");
      await loadRazorpay(); if (!window.Razorpay) throw new Error("Razorpay could not be opened. Please try again.");
      const payment = new window.Razorpay({
        key: order.keyId, amount: order.amount, currency: order.currency, name: "YAFA VANAM", description: `${cart.itemCount} ${cart.itemCount === 1 ? "item" : "items"} from your ritual`, order_id: order.orderId,
        prefill: { name: `${form.firstName} ${form.lastName}`.trim(), email: form.email, contact: `+91${form.phone.replace(/\s/g, "")}` }, theme: { color: "#173426" },
        modal: { ondismiss: () => { setState("idle"); setNotice("Payment was cancelled. Your checkout details are still here."); } },
        handler: async (response: RazorpayResponse) => {
          try {
            const verification = await fetch("/api/payments/razorpay/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(response) });
            ...
            setState("complete"); setNotice(`Payment received for ${result.order_number || "your YAFA VANAM order"}. We’ll send your confirmation once the secure capture is finalised.`);
  ...
  if (state === "complete") return <main className="checkout-page"><section className="checkout-success" aria-live="polite"><span>✓</span><p>YAFA VANAM</p><h1>Payment received.</h1><div>{notice}</div><Link href="/shop">Continue shopping</Link></section></main>;
  ...
  return <main className="checkout-page"><div className="checkout-layout">
    <form className="checkout-main" onSubmit={startPayment} noValidate>
      ...
      <section className="checkout-section checkout-terms"><p>By placing your order, you agree to our <Link href="/terms">Terms &amp; Conditions</Link> and <Link href="/privacy">Privacy Policy</Link>. Payments are processed securely by Razorpay.</p></section>{notice ? <p className={`checkout-notice${state === "processing" ? " is-processing" : ""}`} role="status">{notice}</p> : null}<button className="checkout-submit" type="submit" disabled={!readyToPay} aria-busy={state === "processing"}>{state === "processing" ? "Preparing secure payment…" : `Pay ${formatCatalogPrice(cart.currency, totals.total)}`}</button><footer className="checkout-footer"><Link href="/shipping">Shipping</Link><Link href="/returns">Returns</Link><Link href="/faq">Help</Link></footer>
    </form>
    <aside className={`checkout-summary${summaryOpen ? " is-open" : ""}`} aria-label="Order summary"><button className="checkout-summary__toggle" type="button" onClick={() => setSummaryOpen((value) => !value)} aria-expanded={summaryOpen}><span>Order summary <em>({cart.itemCount})</em></span><strong>{formatCatalogPrice(cart.currency, totals.total)}</strong><b aria-hidden="true">⌄</b></button><div className="checkout-summary__content">{cartStatus === "loading" ? <p className="checkout-summary__empty">Loading your ritual…</p> : null}{cartStatus === "error" ? <p className="checkout-summary__empty">We could not load your bag. Refresh and try again.</p> : null}{cartStatus === "ready" && !cart.items.length ? <p className="checkout-summary__empty">Your bag is waiting for something beautiful. <Link href="/shop">Explore the collection</Link>.</p> : null}{cart.items.map((item) => <article className="checkout-line" key={item.key} aria-busy={busyItemKey === item.key}><div className="checkout-line__image"><Image src={item.image} alt="" fill sizes="72px" /><span>{item.quantity}</span></div><div><h2>{item.name}</h2><p>{[item.size, item.shade].filter(Boolean).join(" · ") || item.productType}</p><div className="checkout-line__quantity" aria-label={`Quantity for ${item.name}`}><button type="button" disabled={busyItemKey === item.key} onClick={() => changeQuantity(item.key, item.quantity - 1)} aria-label={`Decrease ${item.name} quantity`}>−</button><span aria-live="polite">{item.quantity}</span><button type="button" disabled={busyItemKey === item.key || item.quantity >= 20} onClick={() => changeQuantity(item.key, item.quantity + 1)} aria-label={`Increase ${item.name} quantity`}>+</button></div></div><strong>{formatCatalogPrice(item.currency, item.unitPrice * item.quantity)}</strong></article>)}{discountCode ? <div className="checkout-discount checkout-discount--applied"><span><strong>{discountCode}</strong><small>{testCoupons[discountCode]?.label}</small></span><button type="button" onClick={() => { setDiscountCode(""); setDiscountInput(""); setDiscountMessage(""); }} aria-label="Remove discount code">×</button></div> : <><div className="checkout-discount"><label className="visually-hidden" htmlFor="checkout-discount">Discount code or gift card</label><input id="checkout-discount" placeholder="Discount code or gift card" value={discountInput} onChange={(event) => setDiscountInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); applyDiscount(); } }} /><button type="button" onClick={applyDiscount}>Apply</button></div><p className="checkout-discount__hint">Test codes: YAFA20, NATURE15, FLAT500, WELCOME10</p></>}{discountMessage ? <p className={`checkout-discount__message${discountCode ? " is-success" : ""}`} role="status">{discountMessage}</p> : null}<dl className="checkout-totals"><div><dt>Subtotal <span>· {cart.itemCount} {cart.itemCount === 1 ? "item" : "items"}</span></dt><dd>{formatCatalogPrice(cart.currency, cart.subtotal)}</dd></div>{discount ? <div><dt>{discountCode}</dt><dd>−{formatCatalogPrice(cart.currency, discount)}</dd></div> : null}<div><dt>Shipping</dt><dd>{addressReady ? (totals.shipping ? formatCatalogPrice(cart.currency, totals.shipping) : "Complimentary") : "Enter delivery address"}</dd></div><div className="checkout-total"><dt>Total <small>INR</small></dt><dd>{formatCatalogPrice(cart.currency, totals.total)}</dd></div></dl></div></aside>
  </div></main>;
}
