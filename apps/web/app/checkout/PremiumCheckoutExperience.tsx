...
export default function CheckoutExperience() {
  ...
  function applyDiscount() {
    ...
    if (testCoupons[code]) { setDiscountCode(code); setDiscountInput(code); setDiscountMessage(`${code} applied — ${testCoupons[code].label}. Your final discount is verified again by the server before payment.`); }
  ...
  async function startPayment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const orderResponse = await fetch("/api/payments/razorpay/order", { method: "POST", headers: { "Content-Type": "application/json", "X-CSRF-Token": token }, body: JSON.stringify({ shippingMethod, discountCode, customer: form }) });
