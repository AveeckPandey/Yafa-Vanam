...
This release runs the storefront, commerce API, and recommendation engine as
separate ECS Fargate services. It is deliberately not a lift-and-shift of the
old Railway/Vercel instructions: the recommendation engine remains reachable
...
| Recommendation engine | ECS Fargate + Cloud Map service discovery | No |
| Commerce data | RDS PostgreSQL 16 (Multi-AZ in production) | No |
| Sessions/rate limits | ElastiCache Redis 7 | No |
| Private selfie uploads | S3 bucket with block-public-access enabled | No |
...
route handlers, including the server-only advisor bridge. Set `ADVISOR_URL` to
the recommendation engine's Cloud Map DNS name; never expose it as a public
...
3. Create the PostgreSQL database and Redis replication group. Enforce TLS from
   ...
   group.
4. Create a private S3 bucket for selfie uploads with all public access blocked,
   default encryption enabled, lifecycle expiry for uploads, and a task-role-only
   bucket policy. Use the regional S3 endpoint for `YAFA_STORAGE_ENDPOINT`.
5. Create an ACM certificate in the ALB region and validate the `yafavanam.com`
   DNS name. Point Route 53 aliases for the apex (and optional `www`) at the ALB.
6. Create the Cognito User Pool/client and copy its values into the `web`
...
```json
{
  ...
  "ADVISOR_URL": "http://yafa-advisor.yafa.local:8000",
...
```json
{
  ...
  "YAFA_STORAGE_ENDPOINT": "",
  "YAFA_STORAGE_REGION": "ap-south-1",
  "YAFA_STORAGE_BUCKET": "<private-bucket-name>",
  "YAFA_STORAGE_ACCESS_KEY_ID": "",
  "YAFA_STORAGE_SECRET_ACCESS_KEY": "",
  "YAFA_ANALYZER_URL": "http://yafa-advisor.yafa.local:8000",
  "YAFA_INTERNAL_SERVICE_TOKEN": "<same-random-32-plus-character-token>",
...
`yafa/prod/advisor` must contain `YAFA_INTERNAL_SERVICE_TOKEN`; add the RAG
and monitoring keys only when those features are enabled.

The blank S3 endpoint and credentials above are intentional: the API now uses
the ECS task role through the AWS SDK credential chain. Give that role only
`s3:PutObject` and `s3:GetObject` for the selected private bucket. Supply an
endpoint and both static keys only for a non-AWS S3-compatible provider.
...
docker build -f services/recommendation-engine/Dockerfile -t yafa-advisor:release .
```

Create private ECR repositories for `yafa-web`, `yafa-api`, and `yafa-advisor`.
...
- advisor: `GET /health` on port 8000.
...
2. Deploy the advisor, then API, then web services. The Go API automatically
...
5. Confirm the S3 bucket cannot be listed or read anonymously, the advisor has
